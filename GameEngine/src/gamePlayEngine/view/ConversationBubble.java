...
import gamePlayEngine.model.Prop;
import gamePlayEngine.model.ReadBehavior;
import gamePlayEngine.model.Reward;
import gamePlayEngine.model.Size;
import gamePlayEngine.model.Time;
import gamePlayEngine.model.Util;
...
public class ConversationBubble extends JLabel implements TimingTarget{
    ...
    private void drawBubblePointLeft(Graphics g, int x, int y, int width, int height) {
        ...
        // Zac ZHANG: Get Text
        try {
        	stringTextToDisplay = this.prop.getText();
        	if (stringTextToDisplay == null)
        		stringTextToDisplay = " ";
        	try {
            	ReadBehavior readBehavior = new ReadBehavior(prop.getBehavior());
            	if (readBehavior.getModel().trim().equals("Reward")) {
    				Reward reward = GameModel.getGameModelObject().getCharacter().getReward();
    				stringTextToDisplay = stringTextToDisplay + " " + reward.getPoints() + " " ;
    				if(reward.getTitle() != null)
    					stringTextToDisplay += " Your title is " + reward.getTitle();
        		}        	
            } catch(Exception e){
        ...
        // Zac ZHANG: Use new coordinate
        xcoords[0] = x;
        ycoords[0] = y - height + (int)dim.getHeight() - 5;
        xcoords[1] = x + ARROW_WIDTH;
        ycoords[1] = y - height + (int)dim.getHeight() - 5 - ARROW_HEIGHT;
        xcoords[2] = x + ARROW_WIDTH * 2;
        ycoords[2] = y - height + (int)dim.getHeight() - 5 - ARROW_HEIGHT;
        
        // Zac ZHANG: Set the font
        graphics.setStroke(new BasicStroke());// Sets strokes to default, is better.
        try{
        	graphics.setFont(new Font(prop.getFont().toString(), Font.PLAIN, Integer.parseInt(this.prop.getTextSize())));
        }catch(Exception e){
        	graphics.setFont(new Font("Serif",Font.PLAIN,Integer.parseInt("12")));
        }
        
        // Draw the base shape -- the rectangle the image will fit into as well as its outline
        // Zac ZHANG: Get the color from the XML file
        try{
        ...
        g.fillRoundRect(x, y - height - ARROW_HEIGHT, (int)dim.getWidth()-5, (int)dim.getHeight()-5, PADDING * 2, PADDING * 2);
        g.setColor(Color.BLACK);
        // Zac ZHANG: Use the size in XML file to draw the shape
        g.drawRoundRect(x, y - height - ARROW_HEIGHT, (int)dim.getWidth()-5, (int)dim.getHeight()-5, PADDING * 2, PADDING * 2);
    ...
    protected void paintComponent(Graphics g)
    {
    	...
    	paintString(stringTextToDisplay);
    }
    
    public void setOpaque(boolean isOpaque) {
        /* Zac ZHANG: Disable this function
    ...
    public ConversationBubble(Prop prop) {
        super();             
        this.prop = prop;        
        setOpaque(false);
    	...
    	List<gamePlayEngine.model.Timer> timers = GameModel.getGameModelObject().getGameConstants().getTimers();
		int timerValue = 0;
		for(gamePlayEngine.model.Timer timer:timers)
		{
    ...
    private void drawStringRect(Graphics2D graphics, int x1, int y1, int x2, int y2, float interline, String txt) {
